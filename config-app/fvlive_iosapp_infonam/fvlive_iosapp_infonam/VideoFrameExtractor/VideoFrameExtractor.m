...
-(id)initWithVideo:(NSString *)moviePath {
    ...
    // Open video file
    int error;
    if((error = avformat_open_input(&pFormatCtx, [moviePath cStringUsingEncoding:NSASCIIStringEncoding], NULL, NULL)) != 0) {
//  if((error = avformat_open_input(&pFormatCtx, (char*)[moviePath UTF8String], NULL, NULL)) != 0) {
        char buf[1024];
        av_strerror(error, buf, sizeof(buf));
        av_log(NULL, AV_LOG_ERROR, "Couldn't open file err=%s\n", buf);
        goto initError;
    }
